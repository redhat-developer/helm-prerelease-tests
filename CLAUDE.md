...
### Treat planning documents as human-managed too

`testing-farm-integration-plan.md` and any other `*-plan.md` or `*-integration-plan.md` files document human decisions about future work. Do not modify them unless the issue explicitly asks for it. If a glob pattern or file reference in such a document appears to be stale relative to your changes, note it in the PR description rather than editing the document.
